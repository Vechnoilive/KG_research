...
# Cell 1: Установка зависимостей (выполнить в Colab один раз)
...
print("Установки завершены.")

# Cell 2: Импорт и настройки
import os
import json
...
import math
import random
from pyvis.network import Network  # для интерактивной HTML-визуализации

# Путь к входному файлу (df.tsv должен быть в /content/)
INPUT_PATH = "df.tsv"
OUT_DIR = "syntactic_graph_outputs"
os.makedirs(OUT_DIR, exist_ok=True)

# Исключаем служебные части речи и пунктуацию из графа
EXCLUDED_POS = {"ADP", "CCONJ", "SCONJ", "PART", "PUNCT", "SPACE", "SYM", "INTJ"}

# Загружаем большую русскую модель spaCy
nlp = spacy.load("ru_core_news_lg")

# Увеличиваем лимит длины текста для обработки больших документов
nlp.max_length = 2_000_000

print("spaCy model loaded:", nlp.pipe_names)

# Cell 3: Функции для парсинга, генерации id и построения графа
def parse_sentence_spacy(text: str):
    """
    Возвращает tokens и deps для одного предложения:
    tokens: list of {idx (1-based), surface, lemma, pos}
    deps: list of {head, dep, label}
    Исключает служебные слова (предлоги, союзы, частицы, пунктуацию и т.д.).
    """
    doc = nlp(text)
    tokens = []
    deps = []

    # Сначала строим таблицу индексов, исключая ненужные токены
    filtered = [tok for tok in doc if tok.pos_ not in EXCLUDED_POS]
    old_to_new = {tok.i: i + 1 for i, tok in enumerate(filtered)}  # spaCy idx -> 1-based новый индекс
    ...
    for i, tok in enumerate(filtered):
        ...
        # Добавляем зависимость, если головное слово не исключено
        if tok.head.i in old_to_new:
            head_idx = old_to_new[tok.head.i]
        else:
            head_idx = 0  # если голова — служебное слово или root
...
def clean_graph_remove_stopwords(G, nlp_model):
    """
    Удаляет из графа узлы-слова, соответствующие стоп-словам,
    служебным POS и неалфавитным токенам.
    Возвращает новый очищенный граф.
    """
    ...
    print(f"Удаляем {len(nodes_to_remove)} служебных/стоп-узлов из {G.number_of_nodes()}")
    G_clean = G.copy()
...
def safe_str_for_graph(v):
    """Приводит сложные структуры к сериализуемым строкам (для GraphML/GEXF)"""
...
def compute_lemma_pmi(G):
    ...
    # Если нет пар — вернём пустой словарь
    if total_pairs == 0:
        return {}

    # PMI и нормализация в диапазон [0.5, 1.5]
    raw_pmi = {}
    min_pmi = float("inf")
    max_pmi = float("-inf")
    ...
    # Если все PMI одинаковы — вернуть базовый вес 1.0
    pmi_norm = {}
    ...
    # Min-max -> scale to [0.6, 1.4] (вместо [0.5,1.5], немного осторожнее)
    low, high = 0.6, 1.4
    for k, v in raw_pmi.items():
        pmi_norm[k] = low + (v - min_pmi) * (high - low) / (max_pmi - min_pmi)
    return pmi_norm

# Cell 4: Основная функция построения мультиграфа из DataFrame
def build_syntactic_multigraph(df: pd.DataFrame,
    ...
    """
    Строит networkx.MultiDiGraph по выбранным строкам df.
    Возвращает (G, lemma_agg) где lemma_agg — агрегированная статистика меж-леммовых зависимостей.

    Изменения:
    - добавлена фильтрация служебных слов (через parse_sentence_spacy)
    - добавлен прогресс и подсчёт токенов
    - улучшена безопасность при добавлении атрибутов (safe_str_for_graph)
    """
    G = nx.MultiDiGraph()
    ...
    for row_idx, row in tqdm(df_sel.iterrows(), total=len(df_sel), desc="Parsing rows"):
        ...
        # parse через нашу функцию, которая уже фильтрует POS
        try:
            tokens, deps = use_parsing_fn(text)
        except Exception as e:
            print(f"⚠️ Parse error at row {row_idx}: {e}")
            continue

        # если после фильтрации нет токенов — пропускаем
        if not tokens:
            continue

        # создаём узел предложения
        ...
        for d in deps:
            ...
            if head == 0:
                # это корень предложения — не связываем с SENT узлом
                head_inst = None
            else:
...
# Cell 5: Загрузка df.tsv и вызов сборки графа
# Если у тебя df.tsv в другом месте, поправь INPUT_PATH в начале
if not os.path.exists(INPUT_PATH):
    # подсказка: можно загрузить файл с локального ПК
    from google.colab import files
    print("Загрузите df.tsv (файл должен содержать колонки: aid, type, value).")
    uploaded = files.upload()
    # сохранение первого файла как df.tsv
...
# Укажи имена колонок, если они отличаются
type_col = "type"
value_col = "value"
article_col = "aid"  # если в твоём файле другая колонка — измени здесь
...
def visualize_graph_html_fixed_draggable(G,
                                         ...
                                         seed=42):
    """
    Сохраняет интерактивный HTML (pyvis) с фиксированными начальными позициями.
    • Отображает до max_nodes узлов (по степени)
    • Узлы все одного размера
    • Без физики (не двигаются сами)
    • Можно перемещать мышкой (dragNodes=True)
    • Увеличено расстояние между вершинами ×3
    """
    import math
    import networkx as nx
    ...
    if G.number_of_nodes() == 0:
        print("Граф пустой — нечего визуализировать.")
        return

    # 1️⃣ Выбираем топ-N по степени
    top_nodes = sorted(G.degree(), key=lambda x: x[1], reverse=True)[:max_nodes]
    chosen_nodes = {n for n, _ in top_nodes}
    subG = G.subgraph(chosen_nodes).copy()
    print(f"Визуализация подграфа: {subG.number_of_nodes()} узлов, {subG.number_of_edges()} рёбер")

    # 2️⃣ Статическая раскладка (spring_layout)
    pos = nx.spring_layout(subG, seed=seed, iterations=300)
    PIXEL_SCALE = 1800.0  # ← было 600.0 (увеличено ×3)
    MIN_DIST = 240.0      # ← было 80.0 (увеличено ×3)
    MAX_ITERS = 200
    ...
    # Простое раздвигание близких вершин
    ...
    # 3️⃣ Создаём pyvis Network
    net = Network(
        directed=True,
        width="100%",
        height="1000px",  # чуть больше, чтобы вместить
        bgcolor="#ffffff",
        font_color="#000000",
        notebook=False
    )

    # 4️⃣ Добавляем узлы — все одного размера, draggable=True
    node_size = 25
    for n, data in subG.nodes(data=True):
        ...
        net.add_node(
            ...
            y=int(y),
            size=node_size,
            fixed=False,   # можно двигать
            physics=False, # не двигаются сами
            font={"size": 16}
        )
    ...
    print(f"HTML-граф сохранён (расстояние ×3): {output_html_path}")
...
# === Cell X: Полная информация по слову (лемма или поверхностная форма) ===
def show_full_word_info(word: str, G: nx.MultiDiGraph, lemma_agg: dict):
    """
    Выводит детальную информацию по слову:
      • найденные узлы (instance / lemma)
      • их атрибуты и связи
      • статьи, где встречается лемма
      • агрегированные зависимости из lemma_agg
    """
    ...
    if not matches:
        print(f"Слово '{word}' не найдено в графе.")
        return

    print(f"Найдено {len(matches)} узлов для '{word}'.\n")
    ...
    # Информация по отдельным узлам (первые 5)
    for i, node in enumerate(matches[:5], 1):
        data = G.nodes[node]
        print(f"=== Узел {i}/{min(len(matches), 5)}: {node} ===")
    ...
    # Общая статистика по лемме
    print(f"Лемма: {word}")
    if total_deps == 0:
        print("Нет агрегированных зависимостей (слово встречается только изолированно).")
        return

    print(f"Всего синтаксических зависимостей: {total_deps}")
    print(f"Встречается в статьях: {len(docs)}")
    if docs:
        sample = ", ".join(sorted(docs)[:10])
        extra = " ..." if len(docs) > 10 else ""
        print(f"  Примеры: {sample}{extra}")
    ...
    for lbl, targets in dep_by_label.items():
        ...
        print(f"  {lbl:12s} → {len(uniq)} лемм: {sample}{extra}")


# Пример использования
show_full_word_info("семья", G, lemma_agg)

# === Финальные функции поиска с уточняющим вопросом ===

def _get_articles_for_lemma_node(lemma_node: str, G: nx.MultiDiGraph) -> set:
    """Собирает множество article_id из входящих instance_of рёбер (INST → LEM)."""
    articles = set()
...
def get_most_similar_lemma(word: str, G: nx.MultiDiGraph, nlp_model) -> str | None:
    """
    Ищет лемму в графе с максимальным косинусным сходством по векторам spaCy.
    Возвращает ID узла-леммы, если сходство > 0.55.
    """
    try:
    ...
    if best_node and best_sim > 0.55:
        print(f"Похожее слово для «{word}» → «{G.nodes[best_node]['lemma']}» ({best_sim:.2f})")
        return best_node
    return None
...
def find_best_articles_for_question(question: str,
    ...
    """
    Основная функция поиска релевантных статей по вопросу:
      • извлекает леммы вопроса
      • точный поиск + fallback по векторному сходству
      • DFS по lemma_dep с учётом пересечения статей
      • возвращает top-3 статей
      • при близких score задаёт уточняющий вопрос
    """
    doc = nlp_model(question)
    lemmas = [tok.lemma_.lower() for tok in doc if tok.pos_ not in EXCLUDED_POS]
    lemmas = list(dict.fromkeys(lemmas))  # сохраняем порядок, убираем дубли

    print(f"\nВопрос: {question}")
    print(f"Леммы вопроса: {lemmas}")
    ...
    for lemma in lemmas:
        ...
        # Похожее слово, если точного нет
        if not candidates:
        ...
        for node in candidates:
            ...
            # DFS по lemma_dep (без жёсткого ограничения глубины)
            decay_factor = 0.7  # подбирай (0.6-0.85)
    ...
    print("Топ-кандидаты (aid, score):", top_n)

    # Уточняющий вопрос при близких score
    if len(top_n) >= 2:
        best, second = top_n[0][1], top_n[1][1]
        eps = 1e-9
        if (abs(best - second) / (abs(best) + eps)) < 0.10:
            print("Система не уверена между несколькими статьями — нужен уточняющий вопрос.")
            ...
            if df is not None:
                ...
                if len(texts) == 2:
                    ...
                    # Извлечение тематических фраз (ADJ+NOUN или просто NOUN)
                    ...
                    # Корни тематики
                    roots = {"пособие", "выплата", "ребёнок", "семья", "черновик", "школа", "родство"}
                    common = {r for r in roots if any(r in p for p in ph1) and any(r in p for p in ph2)}

                    # Поиск контрастной пары в общей теме
                    ...
                    if pair:
                        a, b = pair
                        print(f"Уточнение: речь о «{a}» или о «{b}»?")
                    else:
                        ...
                        print(f"Уточнение: вы имели в виду «{titles[0]}» или «{titles[1]}»?")
...
def evaluate_question_search(G: nx.MultiDiGraph,
    ...
    """
    Оценивает качество поиска по тестовому набору dfq.tsv.
    Выводит топ-3 статьи с заголовком и сниппетом.
    Считает Accuracy@1, Accuracy@3, Precision, Recall, F1-score, Loss.
    """
    if not os.path.exists(dfq_path):
        print(f"Файл {dfq_path} не найден.")
        return

    dfq = pd.read_csv(dfq_path, sep="\t", dtype=str).fillna("")
    print(f"Загружено {len(dfq)} тестов из {dfq_path}")
    print("Колонки:", list(dfq.columns))

    # Поиск колонок по алиасам
    q_cols = ["question", "q", "query", "user_qu", "вопрос"]
    ...
    if not q_col or not exp_col:
        print("Не найдены обязательные колонки (вопрос / ожидаемый aid).")
        return

    print(f"Колонки: question='{q_col}', expected='{exp_col}'")
    ...
    for idx, row in dfq.iterrows():
        ...
        for rank, (aid, score) in enumerate(results[:3], 1):
            ...
            if rows.empty:
                print(f"{rank}. aid={aid_str} | score={score:.3f} → статья отсутствует в df.tsv")
                continue
        ...
        # === Метрики с учётом равенства скорингов ===
        if found_aids:
    ...
    if total > 0:
        ...
        print("Нет валидных тестов.")

# === Запуск оценки ===
evaluate_question_search(G, df, nlp, DFQ_PATH)

# === Блок: Сохранение исходного графа (все узлы и связи) ===
triplets = []

# Используем исходный граф G (неочищенный)
for u, v, data in G.edges(data=True):
    triplets.append({
        "source": u,
        "target": v,
...
# Сохраняем под именами, явно обозначающими, что это полный граф
...
print(f"Сохранено {len(triplet_df)} триплетов (исходный граф):")
