...
print(f"✅ USER model: {MODEL_NAME} (dim={VECTOR_DIM})")

try:
    nlp = spacy.load("ru_core_news_sm")
    SPACY_OK = True
    print("✅ spaCy RU loaded!")
except:
    nlp = None
    SPACY_OK = False
    print("⚠️ spaCy fallback")
...
def is_meaningful_word(w: str) -> bool:
    ...
    if len(w) < 3 or re.fullmatch(r"[-0-9]+", w):
        return False
    stops = {
        "и", "в", "на", "по", "с", "к", "о", "об", "от", "до", "за", "у",
        "же", "ли", "да", "но", "а", "как", "так", "то", "или", "если",
        "что", "чтобы", "это", "его", "ее", "их", "мы", "вы", "он", "она", "они",
        "при", "из", "без", "про", "для", "над", "под", "не"
    }
...
def extract_triplets_spacy(text: str) -> List[Tuple[str, str, str]]:
    ...
    for token in doc:
        ...
        # Custom: "подтвердить" verb
        if head.lemma_ == "подтвердить" and token.dep_ in ("obj", "dobj"):
            subj_cands = [c for c in head.children if c.dep_.startswith("nsubj")]
...
if not os.path.exists(DFQ_PATH):
    ...
    for idx, row in tqdm(dfq.iterrows(), total=len(dfq), desc="Eval"):
        ...
        if not res:
            print(f"⚠️ Вопрос: {q[:60]}... — нет результатов\n")
            continue
        ...
        if true_aid in article_texts:
            ...
            print(f"\n✅ Ожидаемая статья: AID={true_aid} | {expected_text}...\n")
        else:
            print(f"\n⚠️ Ожидаемая статья: AID={true_aid} не найдена в df\n")
    ...
    print("=" * 90)
    print(f"🎯 Acc@1 = {acc1:.1f}% | Acc@3 = {acc3:.1f}%")
    print("=" * 90)
...
def word_stats(G, word: str, article_texts: dict, top_n: int = 5):
    ...
    print(f"\n📊 '{word}': {node['df']} статей | IDF={node['idf']:.2f}")
    for i, (aid, cnt) in enumerate(counts, 1):
        print(f"{i}. {aid}: {cnt}x | {article_texts[aid][:150]}...")

word_stats(G, "семья", article_texts)  # Test!
word_stats(G, "беременность", article_texts)